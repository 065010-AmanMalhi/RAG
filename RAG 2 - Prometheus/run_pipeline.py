...
def parse_numbered_list(filepath: str) -> list[str]:
    """Parse a numbered list .txt file (e.g., '1. question text') or line-by-line if no numbers."""
    path = Path(filepath)
    if not path.exists():
    ...
    # First, try to see if it's a numbered list
    items = []
    current_item = []
    is_numbered = False
    ...
    if is_numbered:
        ...
        # Fallback: treat each non-empty line as a separate item
        return lines
